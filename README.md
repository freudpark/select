...
## 📊 데이터 저장소 (Firestore) 구조

이 서비스는 데이터를 **'컬렉션(폴더)'**과 **'문서(파일)'** 단위로 관리하는 Firebase Firestore를 사용합니다.

1.  **visitors (참가 로그)**:
    - **역할**: 모든 버튼 클릭(참가 시도) 기록을 담는 '방명록'입니다.
    - **내용**: 참가 시간, 접속 구역, 당첨 여부 등이 기록되어 중복 참가를 방지합니다.
2.  **winners (당첨자 명부)**:
    - **역할**: 보물을 찾은 행운아들의 정보를 모아둔 '금고'입니다.
    - **내용**: 당첨 구역, 입력된 전화번호, 당첨 시각이 개별 파일로 저장됩니다.
3.  **global (전광판)**:
    - **역할**: 서비스 전체의 상태를 보여주는 '대시보드'입니다.
    - **내용**: 현재까지의 총 당첨자 수 등을 실시간으로 관리합니다.
